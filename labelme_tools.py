...
def imgfile2b64(path, quality=70):
    # 打开图像文件并读取二进制数据
...
class class_labelme:
    def __init__(self, dir_img, dir_json, log_func=None):
        self.origin_jsons = glob.glob(os.path.join(dir_json, '**/*.json'), recursive=True)
        db_imgs = get_image_paths(dir_img)
        self.name2img = {get_file_name_without_extension(p): p for p in reversed(db_imgs)}
        self.idx = 0
        self.len = len(self.origin_jsons)
    ...
    def convert(self):
        if self.is_empty():
            return None
        console = []
        console.extend([f"{self.idx+1}/{self.len}"])

        p = self.origin_jsons[self.idx]
        json_out, info = add_img_to_json(p, self.name2img)
        status = save_json(p, json_out)
        self.idx += 1
        console.extend(info)

        return "\n".join(map(str, console))
...
def create_json_zip(dir_json, zip_file, exts=['.json']):
    json_path_list = []
    for ext in exts:
    ...
    print("压缩完成！")
def create_img_zip(dir_json, zip_file, exts=['.jpg']):
    json_path_list = []
    for ext in exts:
        json_path_list.extend(glob.glob(os.path.join(dir_json, f'**/*{ext}'), recursive=True))
...
if __name__ =='__main__':
    ...
    # export_labelme_format_pairs(dir_img, dir_json)
